...
class TestVoiceInTrunk:
    ...
    def test_enabling_sip_registration_emits_host_and_port_on_fresh_config(self):
        # Regression: PATCH against an existing trunk that already has a
        # host on the server side. The local SipConfiguration starts empty,
        # so nothing is in the local _attributes dict. Setting
        # enabled_sip_registration = True must still emit host=None /
        # port=None on the wire — otherwise the server merges the new field
        ...
        assert attrs["enabled_sip_registration"] is True
